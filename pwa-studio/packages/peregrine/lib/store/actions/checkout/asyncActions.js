        ...
        try {
            if (address.region_id) {
                const country = countries.find(
                    ({ id }) => id === address.country_id
                );
               
                const { available_regions } = country;
                const regionData = available_regions.find(
                    ({ id }) => id == address.region_id
                );
               
                address.region = regionData.name;
                address.region_code = regionData.code;
            }
