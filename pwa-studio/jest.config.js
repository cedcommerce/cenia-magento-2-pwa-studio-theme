...
// Reusable test configuration for Ced UI and storefront packages.
const cedPwa = inPackage => ({
    // Expose jsdom to tests.
    browser: true,
    moduleNameMapper: {
        // Mock binary files to avoid excess RAM usage.
        '\\.(jpg|jpeg|png)$': '<rootDir>/packages/ced-ui/__mocks__/fileMock.js',
        // CSS module classes are dynamically generated, but that makes
        // it hard to test React components using DOM classnames.
        // This mapping forces CSS Modules to return literal identies,
        // so e.g. `classes.root` is always `"root"`.
...
const jestConfig = {
    projects: [
        ...
        configureProject('ced-concept', 'Ced Storefront', inPackage =>
            cedPwa(inPackage)
        ),
        configureProject('ced-ui', 'Ced UI', cedPwa),
